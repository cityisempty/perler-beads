...
*   **授权码 token（旧方案）**：通过包含 `?code=<授权哈希>` 的链接访问页面自动激活授权。授权哈希白名单保存在前端 `src/data/tokenHashes.ts`，使用次数保存在浏览器 `localStorage`。
*   **手机号次数（新方案）**：用户首次输入手机号后，会在 Cloudflare D1 中自动建档，默认可下载 10 次。后续前端展示次数、后台查询、后台改次数，都会统一读取同一份 D1 数据。
*   手机号后台管理页位于 `/admin/phones`，可按手机号精确查询，并把该手机号的剩余次数直接改成指定值，例如 `10000`。
...
*   手机号次数改为 Cloudflare D1 存储后，可跨设备同步剩余次数，也方便后台按手机号统一管理额度。
*   图纸生成本身仍然发生在浏览器端，因此“下载完成后再扣次”依然属于轻量级计费方案，不适合作为强对抗型防刷系统。

## 部署到 Cloudflare Pages

项目已内置 Cloudflare Pages 所需脚本，可直接通过 GitHub 仓库连接部署：

1. 创建一个 Cloudflare D1 数据库，并记下数据库名称 / ID。
2. 在 Pages 项目里添加以下绑定：
   *   D1 绑定名：`PHONE_USAGE_DB`
   *   环境变量：`PHONE_USAGE_ADMIN_KEY=<你自己的管理员密钥>`
3. 应用手机号次数表结构：
   ```bash
   npx wrangler d1 execute <你的数据库名> --remote --file=./migrations/0001_phone_access.sql
   ```
4. 安装依赖（Cloudflare 构建时也会自动执行）：
   ```bash
   npm install
   ```
5. 本地构建并检查：
...
6. （可选）本地预览 Cloudflare Pages 行为：
   ```bash
   npm run cf:preview
   ```
   如果你需要在本地联调 D1，请确保本地预览时也把 `PHONE_USAGE_DB` 绑定到同一个数据库或本地 D1 数据库。
7. GitHub 提交代码后，在 Cloudflare Pages 控制台创建站点，选择 GitHub 仓库，设置：
   - Build command：`npm run cf:build`
   - Output directory：`.vercel/output/static`
   - Node compatibility date：`2024-11-06`
8. 如需在命令行部署或做 CI/CD，可使用：
   ```bash
   npm run cf:deploy -- --project-name <cloudflare-project-name>
   ```
9. 绑定域名后：
   *   用户访问站点输入手机号后，会自动在 D1 建立手机号记录并走统一扣次；
   *   如仍需兼容旧授权码，可继续使用 `https://<域名>/?code=<授权哈希>` 激活 token 方案；
   *   管理员可访问 `https://<域名>/admin/phones` 查询手机号并修改剩余次数。
