...
*   **下载成品**:
    ...
    *   **授权计次**: 通过带有 `?code=<SHA-256 哈希>` 的授权链接控制导出次数，每个授权码默认可使用 10 次。

## 下载授权与计次

*   通过包含 `?code=<授权哈希>` 的访问链接自动激活授权，无需输入。
*   每个授权码默认拥有 10 次下载额度，使用次数保存在浏览器 `localStorage` 中。
*   使用 `npm run generate:tokens -- --count=5000` 批量生成 UUID、哈希与授权链接（输出于 `scripts/output/`），再将哈希数组复制到 `src/data/tokenHashes.ts` 即可部署。
*   方案为纯前端校验，适用于轻量级计费场景。若需跨设备同步或更高安全性，可在后续迭代引入后端服务。
